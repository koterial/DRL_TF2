...
class AC_Agent():
    def __init__(self, agent_index, state_shape, action_shape,
        ...
        self.train_actor_1 = AC_Actor(agent_index=self.agent_index,
        ...
        self.train_critic_1 = AC_Critic(agent_index=self.agent_index,
    ...
    def train(self):
        ...
        advantage_batch = tf.stop_gradient(discount_reward_batch - v_batch)
        self.train_critic_1.loss = self.train_critic_1.train(state_batch, discount_reward_batch)
...
class AC_Critic(Critic):
    ...
    @tf.function
    def train(self, state_batch, discount_reward_batch):
        with tf.GradientTape() as tape:
            v_batch = self.get_value(state_batch)
            loss = tf.reduce_mean(tf.square(discount_reward_batch - v_batch))
        gradients = tape.gradient(loss, self.model.trainable_variables)
        gradients = clip_by_local_norm(gradients, self.clip_norm)
        self.opt.apply_gradients(zip(gradients, self.model.trainable_variables))
        return loss
